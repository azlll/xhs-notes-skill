...
- **复盘**：基于发布后数据、截图或公开链接做流量诊断、目标人群与关键词匹配，并生成本地 HTML 可视化看板。
- **降级**：脚本或 Python 不可用时，仍输出 ASCII 示例图或 Markdown 表格复盘报告。
...
| 发布后复盘 | 支持手动数据、截图识别、单篇链接和主页链接，优先输出数据结论、流量漏斗、封面标题点击诊断、目标人群与关键词匹配和下一篇实验卡 |
| HTML 看板 | 生成离线单文件 HTML，内嵌用户图片、流量漏斗、柱状图、饼图、连线图和附录参考评分 |
| 纯文本降级 | `scripts/` 缺失、`.py` 缺失或 Python 不可用时，用 ASCII 示例图和 Markdown 降级版报告继续交付 |
...
| 已发布笔记、截图或链接 | 发布后复盘 | 数据结论、流量漏斗、封面标题点击诊断、关键词匹配、下一篇实验卡、本地 HTML 看板 |
...
- 手动数据：标题、正文、标签、发布时间、曝光、小眼睛/观看/阅读/查看数、点赞、收藏、评论、分享、关注、转化等。
...
`metrics.views` 明确对应小眼睛/观看/阅读/查看数，`metrics.exposure` 才是曝光，二者不能混用。链接解析不到小眼睛/观看数时，Skill 会先问：

```text
我没有拿到小眼睛/观看数，这会影响流量判断。请补充观看数，或回复“跳过”继续做不完整复盘。
```

复盘会优先输出数据结论、流量漏斗、封面标题点击诊断、目标人群与关键词匹配、当前补救动作和下一篇实验卡。六维评分保留但弱化，只作为附录参考。需要网页时，Agent 会把复盘结果整理成 JSON，并调用本地脚本生成离线单文件 HTML：
...
```json
{
  ...
  "traffic": {
    "conclusion": "观看后收藏率偏弱，内容更像情绪共鸣，实用信息需要加强。",
    "funnel_issue": "更可能卡在内容承接和收藏价值，不是单纯曝光不足。",
    "cover_title_diagnosis": "有曝光和 views，可判断封面标题点击效率；标题场景明确，但利益点还可更前置。",
    "current_actions": [{"title": "置顶评论补清单", "action": "补充路线、预算和适合人群。"}],
    "next_iteration": [{"title": "下一篇改成午休自救清单", "change": "封面写 3 个可复制动作。"}]
  },
  "audience_keywords": {
    "target_audience": ["打工人", "午休想放松的人", "城市生活方式人群"],
    "keywords": ["工作日午休", "打工人自救", "咖啡厅放松"],
    "hot_tags": ["小红书咖啡厅", "打工人日常"],
    "title_patterns": ["打工人午休去哪", "工作日自救清单"],
    "match_diagnosis": "标题有场景词，但长尾搜索词还可以更具体。"
  },
  "scores": {
...
- 指标柱状图、互动占比饼图和“内容入口 -> 观看/互动 -> 流量卡点 -> 行动建议”的连线图。
- 当前这篇还能补救什么、下一篇怎么迭代、原因假设和风险边界。
- 附录参考评分：六维评分、六维雷达图和参考总分只放在附录，不作为主结论。
...
要求：先给数据结论、流量漏斗、封面标题点击诊断、目标人群与关键词匹配、当前补救动作和下一篇实验卡；六维评分只放附录参考。
```
...
如果安装环境里的 `scripts/` 目录丢失、`generate_images.py` 缺失、`python3` 不可用或脚本执行失败，Skill 不会中断输出。它会在图片部分改用 ASCII 示例图，并保留可复制 Prompt：
...
生成的 HTML 会包含用户笔记内容、数据结论、流量漏斗、封面标题点击诊断、目标人群与关键词匹配、指标柱状图、互动占比饼图、连线图、当前补救动作和下一篇建议。六维评分、六维雷达图和参考总分只放在附录。

如果 `generate_review_report.py` 缺失、`python3` 不可用、review JSON 不能读取或输出目录不可写，Skill 会输出 `发布后复盘报告（Markdown 降级版）`。降级版会用表格替代 HTML 图表：

```markdown
| 指标 | 数值 | 判断 | 数据缺口 |
| --- | ---: | --- | --- |
| 曝光 | 3200 | 可判断初始分发 | - |
| 小眼睛/观看 | 680 | 可计算观看/曝光转化 | - |
| 收藏率 | 6.2% | 收藏价值一般 | 基于 views |
```

Markdown 降级版会包含数据结论、流量漏斗表、封面标题点击诊断表、目标人群与关键词匹配表、当前补救动作表、下一篇实验卡表、附录参考评分（弱化六维评分表）和风险提醒表，并明确说明未生成真实 HTML 文件。
